...
import { generateWAMessage, generateMessageID, delay } from '../Utils/index.js';
...
class GiftedStatus {
    ...
    async sendGroupStatus(groupJid, content, options = {}) {
        // Use the same path as handleGroupStory — generateWAMessage with full config
        // so that logger, mediaCache, and options are available for thumbnail generation
        // and media upload. Passing only { upload } silently breaks image/audio/video.
        let waMsgContent;
        if (content.message) {
            waMsgContent = content;
        } else {
            waMsgContent = await generateWAMessage(groupJid, content, {
                upload: this.waUploadToServer,
                logger: this.config.logger,
                mediaCache: this.config.mediaCache,
                options: this.config.options,
